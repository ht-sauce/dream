<template>
  <div class="quill">
      ...
      <div class="header">
        <div class="operation">
          <div class="left">
            ...
            <span v-show="editor_type === 'add'" @click="article_modify('1', true)">{{
              draft_tips
            }}</span>
          </div>
          ...
          <el-popover placement="bottom" trigger="manual" v-model="visible">
            <div class="cover-img">
              <div v-show="!cover" @click="$refs.upload.click()" class="upload-cover">
                点击添加封面图片
              </div>
    ...
    <div class="editor">
      ...
      <div class="classify">
        <span>分类：</span>
        <el-checkbox-group @change="save_article" v-model="classify" size="small">
          <el-checkbox-button v-for="item in all_class" :label="item.id" :key="item.id">{{
            item.name
          }}</el-checkbox-button>
        </el-checkbox-group>
      </div>
